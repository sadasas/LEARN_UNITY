...
public class HealingNode : Node
{
    ...
    int Healing()
    {
       ...
       if(_time>0f) _time -= Time.deltaTime;
        ...
        if (_health.CurrentHealth(0) < _health.MaxHealth() && _time<=0f)
        {
            _time = _delay;
            if (_health.CurrentHealth(0)+_healthPlus  <= _health.MaxHealth())
            {
        ...
        else _unitEvent.healingEvent?.Invoke(false);
